...
// @jsturm - motor mount holes changed

include<openscad/configuration.scad>
include<openscad/jonaskuehling-default.scad>

// @jsturm - adjust the "layer_thickness" to your printer settings!!
...
translate([55,38,0])								// POSITIONING
	bearing_washer();


////////// RENDER IDLER /////////////////////////////////////////////////////////////////////
/*translate([-20,-5,16.20])		// POSITIONING
	rotate([0,-90,0])
		wadeidler();*/
...
base_thickness=7;
base_length=75; // base by 5mm extended
base_leadout=25;
...
// layer_thickness=0.25; // defined at the beginning
...
/*
// @jsturm deleted - we don't need it anymore
...
// 		@jsturm making some optics - filling a gap on the upper arm
			translate([40,52 ,0])
				cube([20,5,8]);
...
/*// @jsturm - we don't need the old motor mount at the base
module motor_mount(){
	linear_extrude(height=motor_mount_thickness){
...
/*// @jsturm - we don't need the old motor_mount_holes
...
//define new mounting holes
module hole() {
// screwhead resp. 3mm washer
        translate([0, 0, 0]) {
            cylinder(r = 3.7, h = 3.0, center = false, $fn=24);}
// schaft
        translate([0, 0, 3.0+layer_thickness]) {
            cylinder(r = 1.75, h = 5.05, center = false, $fn=24);
        }
    }

module motor_mount_holes(){

// cutout motor mount hole
translate([51,35.2,0])
	cylinder (h=9, r=11);

// extra cutout at motor mount
translate([54.7,38.5,0])
	cylinder (h=9, r=14.2);
...
// cutout at vertical slot
translate([30,28,0])
	cylinder (h=9, r=6.07);

// fixing to the upper hole
translate(v=[60,58,0])
rotate(a=[0,0,25]){
...
//%cube([12,4,5],center=true);
	...
	difference(){
		...
		//Bearing cutout. Kugellager Aufhaengung
